...
python scripts/draw_lunar_real_map_basemaps.py
python scripts/draw_lunar_task_site_maps.py
...
当前 `run_lunar_ice_bpc.py` 会先尝试小规模 exhaustive direct-DP baseline：当任务数不超过 `direct_baseline_max_tasks` 时，枚举固定 logical graph 上三条路径的所有 per-leg 组合、feasible sortie 和单车 journey，再做 fleet set-partition；此时可输出 `DIRECT_DP_BASELINE_OPTIMAL` 和 `exact_status=EXACT_BASELINE_OPTIMAL`，但 direct-DP 不是 BPC certificate。只有同一节点的 fixed-graph pricing closure 满足完整 task-subset 覆盖、RMP dual 绑定、completion-bound on/off 一致、且最小 reduced cost 非负时，才会生成作用域限定为固定三路径 logical graph 的 true-dual BPC node LP certificate。direct-root LP 只作为 diagnostic audit。20-task 及以上若超出当前 fixed-graph pricing closure 上限，必须 fail closed 或退回 reference / seeded-pool incumbent，并报告 analytic relaxation gap 和 seeded RMP/pricing workload。
...
`draw_lunar_real_map_basemaps.py` 从 preview JSON 生成不带任务点、logical edge 或 route overlay 的科研风格底图，默认输出当前 SP50 基准的 DEM、风险、水冰资源、光照与四联图 PNG/PDF 到 `runs/figures/basemaps/`。传入 `--preview-json data/processed/real_maps/south_pole_30km_scale005_preview.json` 可重现旧 30 km 预览中已有的风险和资源层。

`draw_lunar_task_site_maps.py` 在同一 SP50 水冰资源底图上绘制 20/50 规模 `instance_001` 的任务点和 depot，默认不显示 logical edge、route 或任务编号；加 `--label-tasks` 可显示编号。输出 PNG/PDF 到 `runs/figures/task_sites/`。
...
求解输出现在包含 `bound_ledger`。其中只有 `official_lower_bound=true` 的记录能进入 solution / CSV 的 `lower_bound` 和 `relaxation_gap`；普通大规模 fallback 的 official source 仍是 `analytic_relaxation`。当 fixed-graph pricing closure 满足证书条件时，`fixed_graph_pricing_closure_lp` 会作为 scoped BPC node bound 进入 official lower bound。`direct_fixed_graph_root_lp` 永远只作为 fixed-graph diagnostic audit 记录，不会成为 official BPC bound。`restricted_journey_rmp` 始终只作为 diagnostic bound 写入 `best_diagnostic_bound_*`。

求解输出也包含 `pricing_certificate`。该 artifact 统一管理 no-negative certificate 状态：direct / restricted / seeded diagnostic pricing 仍会 fail closed 为 `NOT_PORTED_TRUE_DUAL_BPC`。只有 fixed-graph closure 或未来 full BPC pricing 明确使用 true-dual source、complete coverage、RMP dual 已绑定且 `min_reduced_cost >= -eps` 时，验证器才允许输出 `CERTIFIED_NO_NEGATIVE`。
...
`exact/certificates/true_dual_pricing_tail.py` 提供 true-dual pricing tail 的单一证书入口。当前 runner 会把 fixed-graph closure 或 direct-pricing / direct-root 诊断 evidence 绑定到这个 artifact；诊断 evidence 继续 fail closed，只有满足严格闭合条件的小规模 fixed-graph closure 会输出 `TRUE_DUAL_PRICING_TAIL_CERTIFIED`。
...
`exact/solver/fixed_graph_pricing_closure.py` 提供 fail-closed 小规模 fixed-graph exhaustive pricing closure 循环：每轮重解 restricted RMP，用 exhaustive direct-label pricing 找 fixed-graph negative reduced-cost columns，加入新列后继续，直到固定图上无负列或达到轮次上限。闭合且通过 dual-binding / completion-bound consistency 检查时，输出 `uses_true_dual_bpc_certificate=true`、`lower_bound_official=true`、`can_certify_no_negative=true`，这只证明 scoped fixed-logical-graph BPC node LP bound，不证明 full tree optimal；否则仍 fail closed。
