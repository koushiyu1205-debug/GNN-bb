"""Development-only counterfactual BPC tree continuation and snapshots."""
...
def _queued_node_from_payload(payload: Mapping[str, object]) -> _QueuedNode:
    context = branch_context_from_payload(
        dict(payload.get("branch_context") or {})
    )
    cut_context = cut_context_from_payload(
        dict(payload.get("cut_context") or {})
    )
    cut_lineage = cut_lineage_from_payload(
        dict(payload.get("cut_lineage") or {})
    )
    if cut_lineage.validate_context(cut_context):
        raise ValueError(
            "certified-tree queued node cut lineage mismatch"
        )
    node_id = str(payload.get("node_id") or "")
    if not node_id:
        raise ValueError("certified-tree queued node has no identity")
    return _QueuedNode(
        node_id=node_id,
        parent_node_id=(
            None
            if payload.get("parent_node_id") is None
            else str(payload.get("parent_node_id"))
        ),
        depth=int(payload.get("depth") or 0),
        context=context,
        branch_pair=(
            None
            if payload.get("branch_pair") is None
            else dict(payload.get("branch_pair") or {})
        ),
        branch_sense=(
            None
            if payload.get("branch_sense") is None
            else str(payload.get("branch_sense"))
        ),
        inherited_lower_bound=_float_or_none(
            payload.get("inherited_lower_bound")
        ),
...
def solve_b3_branch_price_tree_baseline(
    ...
    *,
    ...
    development_certified_root_node: Mapping[str, object] | None = None,
    development_certified_tree_state: Mapping[str, object] | None = None,
    ...
    if (
        development_certified_root_node is not None
        and development_certified_tree_state is not None
    ):
        raise ValueError(
            "certified-root and certified-tree continuation are exclusive"
        )
    proof_debt = ProofDebtQueue()
    certified_tree_state = (
        None
        if development_certified_tree_state is None
        else dict(development_certified_tree_state)
    )
    if (
        development_certified_root_node is None
        and certified_tree_state is None
    ):
        ...
        nodes: list[dict] = []
        next_node_index = 1
    elif development_certified_root_node is not None:
        if not provided_initial_columns:
            raise ValueError(
                "certified-root continuation requires bound initial columns"
            )
        root_path = _branch_signature(
            branch_context_from_payload(
                dict(
                    development_certified_root_node.get(
                        "branch_context"
                    )
                    or {}
                )
        ...
        requested_root_rank = branch_rank_by_path.get(
            root_path,
            default_branch_rank,
        )
        root_node, queued_children = (
            _development_certified_root_continuation(
                node=dict(development_certified_root_node),
                requested_rank_index=requested_root_rank,
                active_live_policy=active_live_policy,
            )
        )
        nodes = [root_node]
        queue = list(queued_children)
        next_node_index = 3
    else:
        if not provided_initial_columns:
            raise ValueError(
                "certified-tree continuation requires bound initial columns"
            )
        assert certified_tree_state is not None
        target_node = dict(
            certified_tree_state.get("target_node") or {}
        )
        target_path = _branch_signature(
            branch_context_from_payload(
                dict(target_node.get("branch_context") or {})
            )
        )
        requested_target_rank = branch_rank_by_path.get(
            target_path,
            default_branch_rank,
        )
        first_child_index = int(
            certified_tree_state.get("next_node_index") or 0
        )
        if first_child_index <= 0:
            raise ValueError(
                "certified-tree continuation has invalid next node index"
            )
        certified_target, target_children = (
            _development_certified_node_continuation(
                node=target_node,
                requested_rank_index=requested_target_rank,
                active_live_policy=active_live_policy,
        ...
        nodes = [
            dict(row)
            for row in certified_tree_state.get("processed_nodes") or ()
        ]
        queue = [
            _queued_node_from_payload(dict(row))
            for row in certified_tree_state.get(
        ...
        } | {row.node_id for row in queue}
        target_node_id = str(certified_target.get("node_id") or "")
        child_node_ids = {row.node_id for row in target_children}
        if (
            not target_node_id
            or target_node_id in known_node_ids
            or child_node_ids.intersection(known_node_ids)
            or len(child_node_ids) != 2
        ):
            raise ValueError(
                "certified-tree continuation node identities overlap"
            )
        nodes.append(certified_target)
        queue.extend(target_children)
        next_node_index = first_child_index + 2
    if certified_tree_state is None:
        incumbent_objective = _float_or_none(b0_direct.objective)
        incumbent_source = (
            "B0_DIRECT_DP_FEASIBLE_INCUMBENT"
            if incumbent_objective is not None
            else ""
        )
        incumbent_columns: tuple[JourneyColumn, ...] = tuple(
            b0_direct.journeys
        )
    else:
        incumbent_objective = _float_or_none(
            certified_tree_state.get("incumbent_objective")
        )
        incumbent_source = str(
            certified_tree_state.get("incumbent_source") or ""
        )
        incumbent_columns = tuple(
            certified_tree_state.get("_incumbent_columns") or ()
        )
    ...
    restricted_mip = (
        {
            "status": "CERTIFIED_PREFIX_STATE_REUSED",
            "objective": incumbent_objective,
            "selected_column_count": len(incumbent_columns),
            "known_column_count": len(global_columns_by_signature),
            "wall_time_sec": 0.0,
            "used_as_upper_bound_only": True,
            "_columns": incumbent_columns,
        }
        if certified_tree_state is not None
        else _restricted_column_mip_incumbent(
            data,
        ...
        if status == "NODE_LP_CERTIFIED":
            ...
            if bool(integer_candidate.get("matches_node_lp_bound")):
                ...
                if candidate is None:
                    ...
                    if development_branch_snapshot_callback is not None:
                        development_branch_snapshot_callback(
                            {
                                "schema_version": (
                                    "lunar_ice_bpc.development_branch_"
                                    "runtime_snapshot.v1"
                                ),
                                "target_node": dict(node),
                                "processed_nodes": tuple(
                                    dict(row) for row in nodes
                                ),
                                "open_nodes_before_target_branch": tuple(
                                    queue
                                ),
                                "global_columns": tuple(
                                    global_columns_by_signature.values()
                                ),
                                "target_active_columns": (
                                    node_priced_columns
                                ),
                                "incumbent_objective": (
                                    incumbent_objective
    ...
    payload["development_certified_root_continuation"] = bool(
        development_certified_root_node is not None
    )
    payload["development_certified_root_pricing_reused"] = bool(
        development_certified_root_node is not None
    )
    payload["development_certified_tree_continuation"] = bool(
        certified_tree_state is not None
    )
    payload["development_certified_tree_prefix_pricing_reused"] = bool(
        certified_tree_state is not None
    )
    return payload
...
def _solve_b3_node(
    ...
    *,
    ...
    node["live_cut_policy_hash"] = str(
        engine.get("live_cut_policy_hash")
        or active_live_policy.policy_hash
    )
    node["separator_policy_version"] = str(
        engine.get("separator_policy_version") or ""
    )
...
def _development_certified_root_continuation(
    *,
    node: dict,
    requested_rank_index: int,
    active_live_policy: LiveSriPolicy,
) -> tuple[dict, tuple[_QueuedNode, ...]]:
    if not (
        str(node.get("node_id") or "") == "node_000"
        and node.get("parent_node_id") is None
        and int(node.get("depth") or 0) == 0
    ):
        raise ValueError(
            "development certified root snapshot identity mismatch"
        )
    branched, children = _development_certified_node_continuation(
        node=node,
        requested_rank_index=requested_rank_index,
        active_live_policy=active_live_policy,
        first_child_index=1,
    )
    branched["development_certified_root_snapshot_reused"] = True
    branched["development_certified_root_pricing_reused"] = True
    return branched, children


def _development_certified_node_continuation(
    *,
    node: dict,
    requested_rank_index: int,
    active_live_policy: LiveSriPolicy,
    ...
    """Validate and branch one already-certified development node snapshot.

    This entry point is deliberately separate from the production root path.
    It may skip repeated pricing only after the exact node-LP certificate,
    cut context, lineage, and legal top-3 universe have all been validated.
    Descendant nodes still run the normal exact pricing and certificate path.
    """

    ledger = node.get("certificate_ledger") or {}
    if not (
        bool(str(node.get("node_id") or ""))
        and int(node.get("depth") or 0) >= 0
        and str(node.get("requested_node_status") or "")
        == "NODE_LP_CERTIFIED"
        and str(node.get("certificate_scope") or "")
        == "BPC_NODE_LP_CERTIFIED"
        and str(node.get("pricing_state") or "")
        == "CERTIFIED_NO_NEGATIVE"
        and bool(node.get("node_lp_bound_official"))
        and bool(node.get("uses_true_dual_bpc_certificate"))
        and bool(node.get("manual_rc_audit_pass"))
        and bool(node.get("pricing_rc_audit_pass"))
        and bool(node.get("final_judge_certifying_proof_kind"))
        and bool(ledger.get("valid"))
        and int(node.get("proof_debt_unreleased_count") or 0) == 0
    ):
        raise ValueError(
            "development certified node snapshot is not exact-safe"
        )
    if (
        str(node.get("live_cut_policy_hash") or "")
        != active_live_policy.policy_hash
    ):
        raise ValueError(
            "development certified node live-cut policy mismatch"
        )

    node_context = branch_context_from_payload(
        dict(node.get("branch_context") or {})
    )
    cut_context = cut_context_from_payload(
        dict(node.get("cut_context") or {})
    )
    cut_lineage = cut_lineage_from_payload(
        dict(node.get("cut_lineage") or {})
    )
    if cut_lineage.validate_context(cut_context):
        raise ValueError(
            "development certified node cut lineage mismatch"
        )
    if (
        str(node.get("active_cut_context_hash") or "")
        != cut_context.active_cut_context_hash
    ):
        raise ValueError(
            "development certified node cut-context hash mismatch"
        )

    candidates = list(
        (node.get("fractional_branch_probe") or {}).get("candidates")
        or ()
    )
    if len(candidates) != 3:
        raise ValueError(
            "development certified node must bind exactly the P0 top-3"
        )
    candidate_ids = tuple(
    ...
    if (
        ...
        or int(node.get("guidance_filter_count") or 0) != 0
    ):
        raise ValueError(
            "development certified node legal universe mismatch"
        )

    selected, selected_rank, fallback = _selected_fractional_candidate(
        node,
        requested_rank_index=requested_rank_index,
    )
    if selected is None or selected_rank is None or fallback:
        raise ValueError(
            "development certified node rank did not select bound top-3"
        )
    node_bound = _float_or_none(node.get("node_lp_bound"))
    if node_bound is None:
        raise ValueError(
            "development certified node has no official LP bound"
        )

    node_id = str(node["node_id"])
    node_depth = int(node.get("depth") or 0)
    if int(first_child_index) <= 0:
        raise ValueError(
            "development certified node child index is invalid"
        )
    child_ids = [
        f"node_{int(first_child_index) + offset:03d}"
    ...
    branched.update(
        {
            ...
            "selected_branch_candidate": {
                "task_a": str(selected["task_a"]),
                "task_b": str(selected["task_b"]),
            },
            "selected_branch_candidate_source": (
                "development_certified_node_fixed_shortlist_rank"
                if int(selected_rank) != 0
                else "ryan_foster_fractional_mass"
            ),
            "development_branch_path_signature": list(
                _branch_signature(node_context)
            ),
            "development_branch_requested_rank_index": int(
                requested_rank_index
            ),
            "development_branch_selected_rank_index": int(
                selected_rank
            ),
            ...
            "tree_node_wall_time_sec": 0.0,
            "tree_elapsed_sec_at_exit": 0.0,
            "development_certified_node_snapshot_reused": True,
            "development_certified_node_pricing_reused": True,
        }
    )
    children = []
    for index, (child_context, branch_sense) in enumerate(
        _child_contexts(node_context, selected),
    ):
        children.append(
            _QueuedNode(
                node_id=child_ids[index],
    ...
    return branched, tuple(children)
