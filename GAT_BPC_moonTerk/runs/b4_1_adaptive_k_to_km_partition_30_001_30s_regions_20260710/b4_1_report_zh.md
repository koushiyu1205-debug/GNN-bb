...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_adaptive_k_to_km_partition_30_001_30s_regions_20260710/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_adaptive_k_to_km_partition_30_001_30s_regions_20260710/b4_1_summary.json`
...
- Partition candidate audit rows: `1`；gate pass `1`；gate fail `0`；candidate no-negative `1`；redline fail `0`。
...
- Partition model size: max variables `6009`；max constraints `18078`；max mean variables `3938.202073`；max mean constraints `11779.067358`。
- Partition slot pruning: max feasible assignments `275`；pruned assignments sum `18057`；pruned arc options sum `2183`。
- Partition dual/active scope mismatch rows: `193`；max active-pool-after-dual delta `33`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `13`；OK `5`；exact OK `0`；residual OK `5`。
- Residual task-count partition: enabled rows `1`；expected `30`；observed `30`；proven `30`；incomplete `0`；negative `0`；missing `0`。
