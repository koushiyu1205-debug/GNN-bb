...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k10_active_sortie_m2_m5_30s_probe/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k10_active_sortie_m2_m5_30s_probe/b4_1_summary.json`
...
- Partition model size: max variables `4156`；max constraints `12020`；max mean variables `3194.75`；max mean constraints `9232.75`。
- Partition slot pruning: max feasible assignments `126`；pruned assignments sum `49`；pruned arc options sum `34`。
- Partition dual/active scope mismatch rows: `0`；max active-pool-after-dual delta `0`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `2`；OK `2`；exact OK `0`；residual OK `2`。
