...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k17_k30_active_sortie_pairconflict_full_30s_probe/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k17_k30_active_sortie_pairconflict_full_30s_probe/b4_1_summary.json`
...
- Partition model size: max variables `6656`；max constraints `19549`；max mean variables `3446.933131`；max mean constraints `10070.012158`。
- Partition slot pruning: max feasible assignments `275`；pruned assignments sum `52479`；pruned arc options sum `3298`。
...
- Residual task-count partition: enabled rows `1`；expected `30`；observed `14`；proven `14`；incomplete `0`；negative `0`；missing `16`。
