...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k1_k5_active_sortie_split_boundcert_probe/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k1_k5_active_sortie_split_boundcert_probe/b4_1_summary.json`
...
- Partition model size: max variables `4156`；max constraints `12023`；max mean variables `2837.388889`；max mean constraints `8192.777778`。
- Partition slot pruning: max feasible assignments `126`；pruned assignments sum `236`；pruned arc options sum `136`。
- Partition dual/active scope mismatch rows: `0`；max active-pool-after-dual delta `0`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `12`；OK `12`；exact OK `3`；residual OK `9`。
- Residual task-count partition: enabled rows `1`；expected `30`；observed `5`；proven `5`；incomplete `0`；negative `0`；missing `25`。
