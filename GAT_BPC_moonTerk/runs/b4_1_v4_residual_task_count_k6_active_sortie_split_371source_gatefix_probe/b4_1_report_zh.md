...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k6_active_sortie_split_371source_gatefix_probe/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k6_active_sortie_split_371source_gatefix_probe/b4_1_summary.json`
...
- Partition model size: max variables `4576`；max constraints `13262`；max mean variables `4576.0`；max mean constraints `13262.0`。
- Partition slot pruning: max feasible assignments `144`；pruned assignments sum `216`；pruned arc options sum `72`。
- Partition dual/active scope mismatch rows: `0`；max active-pool-after-dual delta `0`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `4`；OK `4`；exact OK `0`；residual OK `4`。
