...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k5_triple_after_replacement_merge_audit_refresh/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k5_triple_after_replacement_merge_audit_refresh/b4_1_summary.json`
...
| --- | --- | --- | ---: | ---: | ---: | ---: | ---: | ---: | ---: |
| B | B4.1_partition_candidate_audit | V4_current_triple_time_window_infeasible | 1 | 0 | 0 | 0 | 0 | None | None |
...
- Partition candidate top issue: `unproven_residual_task_count_region`；issue counts `{"incomplete_residual_task_count_region":1,"missing_residual_task_count_region":1,"negative_bound_residual_task_count_region":1,"unproven_residual_task_count_region":1}`。
...
- Partition model size: max variables `4156`；max constraints `12046`；max mean variables `1484.25`；max mean constraints `4246.5`。
- Partition slot pruning: max feasible assignments `126`；pruned assignments sum `53`；pruned arc options sum `14`。
- Partition dual/active scope mismatch rows: `0`；max active-pool-after-dual delta `0`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `4`；OK `4`；exact OK `3`；residual OK `1`。
- Residual task-count partition: enabled rows `1`；expected `30`；observed `1`；proven `0`；incomplete `1`；negative `0`；missing `29`。
- Partition refreshed dual rows: `1`；refresh negative count `0`；refresh min RC `-1e-09`。
