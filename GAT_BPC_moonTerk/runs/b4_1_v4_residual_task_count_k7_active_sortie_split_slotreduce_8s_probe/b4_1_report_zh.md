...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k7_active_sortie_split_slotreduce_8s_probe/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k7_active_sortie_split_slotreduce_8s_probe/b4_1_summary.json`
...
- Partition model size: max variables `4996`；max constraints `14502`；max mean variables `3193.0`；max mean constraints `9244.571429`。
- Partition slot pruning: max feasible assignments `162`；pruned assignments sum `133`；pruned arc options sum `58`。
- Partition dual/active scope mismatch rows: `0`；max active-pool-after-dual delta `0`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `3`；OK `3`；exact OK `0`；residual OK `3`。
