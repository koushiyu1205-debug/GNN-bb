...
- CSV rows: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k5_active_sortie_split_gatefix_probe/b4_1_rows.csv`
- JSON summary: `/home/kai/work/GAT_BPC_moonTerk/runs/b4_1_v4_residual_task_count_k5_active_sortie_split_gatefix_probe/b4_1_summary.json`
...
- Partition candidate top issue: `unproven_residual_task_count_region`；issue counts `{"incomplete_residual_task_count_region":1,"missing_residual_task_count_region":1,"unproven_residual_task_count_region":1}`。
...
- Partition model size: max variables `4156`；max constraints `12023`；max mean variables `2820.125`；max mean constraints `8131.875`。
- Partition slot pruning: max feasible assignments `126`；pruned assignments sum `149`；pruned arc options sum `62`。
- Partition dual/active scope mismatch rows: `0`；max active-pool-after-dual delta `0`；negative RC audit fail `0`。
- Partition region MIP-start: enabled `7`；OK `7`；exact OK `3`；residual OK `4`。
