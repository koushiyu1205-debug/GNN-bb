...
| 1 P8--P9 | State three contributions—model, learning-guided exact algorithm, and benchmark/evaluation package—followed by the open learning-evidence boundary and paper organization | CL001--CL011, CL035--CL042; EV027, EV036--EV037 | `MIXED, QUALIFIED` | Combines proof and guidance as one algorithm contribution; reports only the frozen no-task-wait baseline |
...
| 3 opening--3.1 P5 | Define the 50 km by 50 km forward-looking benchmark, candidate-site/service roles, logical nodes and typed path options; display distance in manuscript (1); trace lunar terrain/illumination summaries to immutable stored attributes without displaying uncalibrated mixing weights; and restrict exact same-endpoint substitution to equal-travel-time options because task-site waiting is prohibited | EV009--EV011; EV029, EV032, EV035; `domain/real_maps.py::_path_metrics`; LS03--LS05, LS14 | `SUPPORTED GENERATOR + REVISED TIMING SCOPE` | Establishes how lunar layers enter the fixed discrete solution space while keeping slower timing alternatives that may be needed for no-wait feasibility |
...
| 6.1 | Report the 5/10/20/30 no-task-wait implementation baseline with root-only SRI-3 | EV036--EV037 | `FROZEN RESULT` | Establishes revised-model closure and the descriptive scale trend without a cross-model speed claim; does not replace the pending paired L0 learning control |
