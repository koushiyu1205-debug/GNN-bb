...
| INT-2 | Explain low solar elevation, PSRs, thermal burden, static task windows, cumulative shadow exposure and alternative-path trade-offs as one lunar operating problem | C042, C054, C055; EV029, EV032, EV034; LS01--LS02, LS07, LS13 | none | CITATION + NOW-PROOF | Direct sunlight is not a service prerequisite; communication remains exogenous; shadow exposure, energy and risk remain distinct |
...
| INT-6 | State four contributions and the section order in one closing paragraph | CL001--CL011, CL035--CL039; EV002--EV011, EV027--EV033 | none | MIXED | Seasonal comparison is an evaluation protocol, not a completed result; learning and phase effects remain open |
...
| PROB-5 | Define trip feasibility and reconstruction: depot return, time windows, selected-arc precedence, energy, shadow exposure, load, service risk, operating cost, weighted completion, recharge and trip sequencing | EV002; source S006; `gurobi_compact.py`; manuscript (2)–(7) | TAB01; equation block | NOW-PROOF | Keep every implemented defining transition and the 0.01 service-risk factor consistent with pricing; separate implementation strengthenings |
| PROB-5A | Display the core trip-level MILP families: depot/task flow, activation, task-count and uniqueness links, binary domains, subtour elimination, time propagation, resource limits, recharge and inter-trip compatibility | `gurobi_compact.py`; `columns.py`; Native SPPRC visited-task state; manuscript (4a)–(7) | equation blocks | NOW-PROOF | These define feasible columns in \(\mathcal P(n)\), not additional route-master rows; do not enumerate optional redundant tightenings as base constraints |
...
| Unit | Paragraph Function | Evidence Anchor | Visual/Table | Maturity | Guardrail |
|---|---|---|---|---|---|
| METH-19 | Define the node integer-feasible set and prove canonical-route completeness under time-independent resources, nonnegative completion weights and no reward for waiting | EV002, EV009, EV029, EV033; manuscript (24); CL039 | theorem block | NOW-PROOF | Hourly environmental states must be aggregated before optimization; each resulting instance preserves the lemma, whereas departure-time-dependent resources inside one solve can invalidate it if delay changes a path transition |
