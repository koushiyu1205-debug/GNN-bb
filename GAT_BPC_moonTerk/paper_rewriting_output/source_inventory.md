...
| S055 | `src/lunar_ice_bpc/domain/real_maps.py`; `src/lunar_ice_bpc/exact/core/columns.py`; `src/lunar_ice_bpc/exact/core/journey.py`; `src/lunar_ice_bpc/exact/core/objective.py`; `src/lunar_ice_bpc/exact/solver/journey_driver.py`; `src/lunar_ice_bpc/exact/solver/gurobi_compact.py` | Python group | Detailed lunar path, trip, multi-trip route, no-task-wait timing, normalization and objective mathematics | B | implemented and targeted-test verified | Source anchors for stable resource/objective equations and arrival-equals-service-start timing |
...
| S057 | `CODEX_lunar_gat_bpc_exact_algorithm_design.md`; `src/lunar_ice_bpc/exact/master/journey_rmp.py`; `src/lunar_ice_bpc/exact/bpc/solver/branch_tree_solver.py`; `src/lunar_ice_bpc/exact/core/branching.py`; `src/lunar_ice_bpc/exact/core/cuts.py`; `native/lunar_spprc/src/native_pricer.cpp` | Design and source group | Overall conditional exactness proof chain and fail-closed terminal semantics | B | no-task-wait pricing realized; explicit no-pair incompleteness remains | Source anchors for Lemmas 1--5, Theorem 1 and equations (24)--(27) |
...
| S059 | `runs/frozen_native_live_sri_p0_no_task_wait_baseline_v3_20260725/baseline_freeze_manifest.json`; `verify_freeze.py`; frozen source/config/native/test snapshots | Frozen exact package | Content-addressed no-task-wait implementation, numerical policy and reproducibility binding | A | verified, zero mismatches | Activates implementation-level timing and exactness claims |
| S060 | `runs/frozen_native_live_sri_p0_no_task_wait_baseline_v3_20260725/performance/full80_summary_snapshot.json`; row and report snapshots | Frozen experiment | 80 strict-cold single-repeat scale-5--30 no-task-wait exact closures and descriptive timing | A | 80/80 exact; 80/80 correctness | Activates Table 1 and the bounded baseline contribution |
...
| M006 | Paired mission-epoch instance and four-phase comparison package | `TBD` | No seasonal operating-phase result |
