#!/usr/bin/env python3
"""Freeze the independent QG2 candidate only after every formal gate passes."""
...
FREEZE = (
    RUN_ROOT / "qg2_clean_v2_candidate_finalizer_freeze_storage_cap_v3.json"
)
FORMAL_STATE = RUN_ROOT / "qg2_clean_v2_formal_controller_state.json"
FORMAL_RESULT = RUN_ROOT / "formal_full20_acceptance_qg2_clean_v2.json"
...
PRE_FREEZE_AUDIT = RUN_ROOT / "p0v5_qg2_v2_pre_freeze_audit.json"
FINAL_AUDIT = RUN_ROOT / "p0v5_qg2_v2_completion_audit.json"
CANDIDATE = RUN_ROOT / "P0V5_QG2_LABEL_STATE_GAT_candidate_freeze.json"
STATE = RUN_ROOT / "qg2_clean_v2_candidate_finalizer_state.json"
...
NATIVE_EXTENSION = next(iter(sorted(
...
def main() -> int:
    ...
    if not bool(manifest_payload.get("deployment_authorized")):
        raise SystemExit("QG2 finalizer manifest is not deployment-authorized")
    ...
    if CANDIDATE.exists() or PRE_FREEZE_AUDIT.exists():
        raise SystemExit("QG2 candidate finalizer refuses overwrite or resume")
...
def _candidate_payload(*, calibration, manifest, manifest_payload, checkpoint):
    frozen = {
        _relative(FREEZE): _sha256(FREEZE),
        _relative(CONFIG): _sha256(CONFIG),
        _relative(COLLECTION_FREEZE): _sha256(COLLECTION_FREEZE),
        _relative(ORACLE_FREEZE): _sha256(ORACLE_FREEZE),
        _relative(POST_ORACLE_FREEZE): _sha256(POST_ORACLE_FREEZE),
        _relative(E2E_FREEZE): _sha256(E2E_FREEZE),
        _relative(FORMAL_FREEZE): _sha256(FORMAL_FREEZE),
        _relative(RUNTIME_SOURCE): _sha256(RUNTIME_SOURCE),
        _relative(MODEL_SOURCE): _sha256(MODEL_SOURCE),
        _relative(CALIBRATION_REPORT): _sha256(CALIBRATION_REPORT),
        _relative(manifest): _sha256(manifest),
        _relative(checkpoint): _sha256(checkpoint),
        _relative(E2E_RESULT): _sha256(E2E_RESULT),
        _relative(FORMAL_RESULT): _sha256(FORMAL_RESULT),
        _relative(PRE_FREEZE_AUDIT): _sha256(PRE_FREEZE_AUDIT),
    }
    if NATIVE_EXTENSION is None or not NATIVE_EXTENSION.is_file():
        raise SystemExit("QG2 finalizer Native extension missing")
    frozen[_relative(NATIVE_EXTENSION)] = _sha256(NATIVE_EXTENSION)
    collection = _load(COLLECTION_FREEZE)
    action_policy_hashes = {
        str(scale): str(digest)
        for scale, digest in dict(collection.get(
            "required_exact_action_policy_hashes_by_scale"
        ) or {}).items()
        if str(digest)
    }
    if set(action_policy_hashes) != {"30", "50"}:
        raise SystemExit(
            "QG2 finalizer requires scale30 and scale50 exact-action hashes"
        )
    return {
        "schema_version": "lunar_ice_bpc.p0v5_qg2_candidate_freeze.v1",
        "status": "FROZEN_EXPERIMENT_CANDIDATE",
        "model_id": "P0V5_QG2_LABEL_STATE_GAT",
        ...
        "source_engine_hash": str(collection.get("source_engine_hash") or ""),
        "exact_action_policy_hashes_by_scale": {
            str(scale): str(digest)
            for scale, digest in sorted(
                action_policy_hashes.items(),
...
def _valid_acceptance(payload, *, mode, scales, path, expected_sha256="") -> bool:
    return bool(
...
def _run_audit(*, output: Path, pre_final_freeze: bool) -> int:
    command = [
        sys.executable,
        str(ROOT / "scripts/audit_p0v5_qg2_v2_completion.py"),
        "--run-tests",
        "--output", str(output),
    ]
    if pre_final_freeze:
    ...
    return subprocess.run(
        command, cwd=ROOT, env=_python_env(), check=False
    ).returncode
...
def _matching_formal_controller_alive(pid: int) -> bool:
    ...
    try:
        ...
        return False
    return "run_p0v5_qg2_clean_v2_formal_after_e2e.py" in command
...
def _python_env() -> dict[str, str]:
    ...
    env["PYTHONPATH"] = (
        f"{ROOT / 'src'}:{ROOT / 'build/native-spprc-bidirectional-feasibility-v1'}"
    )
    return env
...
def _validate_freeze() -> None:
    ...
    if payload.get("schema_version") != (
        ...
        raise SystemExit("QG2 candidate finalizer freeze schema mismatch")
    if bool(payload.get("production_default")):
        raise SystemExit("QG2 candidate finalizer cannot change production")
...
def _state(status: str, **extra) -> None:
    payload = {
        "schema_version": "lunar_ice_bpc.p0v5_qg2_candidate_finalizer_state.v1",
        "status": status,
        **extra,
    }
