#!/usr/bin/env python3
"""Immutable artifact helpers for multi-resolution frontier V9R1."""
...
CONFIG = ROOT / "configs/experiments/p0v5_multires_frontier_observability_v9r1.json"
DEFAULT_RUN_ROOT = ROOT / "runs/p0v5_multires_frontier_observability_v9r1_20260818"
...
def write_once(path: str | Path, payload: Any) -> None:
    ...
    if target.exists():
        if target.read_text(encoding="utf-8") != encoded:
            raise SystemExit(f"immutable V9R1 artifact drift:{target}")
        return
...
def verify_freezes(run_root: Path) -> None:
    ...
    for root_key, artifacts in imported["source_artifacts"].items():
        source_root = Path(imported[root_key])
        for relative, digest in artifacts.items():
            path = source_root / relative
            if not path.is_file() or sha256(path) != digest:
                raise SystemExit(f"SOURCE_EVIDENCE_DRIFT:{root_key}:{relative}")
...
def assert_active(run_root: Path, stage: str) -> dict[str, Any]:
    ...
    if bool(state.get("terminal")):
        raise SystemExit("terminal V9R1 chain forbids writers")
...
def update_state(run_root: Path, stage: str, status: str, **detail: Any) -> None:
    state = load(run_root / "state.json")
    if bool(state.get("terminal")):
        raise SystemExit("terminal V9R1 chain forbids transition")
...
def write_terminal(
    ...
    write_once(run_root / "terminal_decision.json", {
        "schema_version": "lunar_ice_bpc.p0v5_multires_frontier_terminal.v1",
        "decision": "FAIL", "reason": reason, "detail": dict(detail),
        "next_authorized_stage": "MULTI_TIMEPOINT_LATE_SWITCH_ORACLE",
