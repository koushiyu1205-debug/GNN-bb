#!/usr/bin/env python3
"""Build a sector-wave GAT embedding kNN/OOD audit-only smoke runbook.

The builder itself is read-only.  It emits:

1. a counterfactual replay capture command for 20-task sector-wave instances;
2. a capture-validation command that builds a GAT validation dataset and runs
   the GAT embedding + kNN/OOD safety shell.

The runbook does not enable workers, certificates, or official-bound effects.
"""
...
DEFAULT_OUTPUT_DIR = Path(
    "BPC_future/results/gat_embedding_sector_wave_smoke_runbook_20260614"
)
DEFAULT_REPORT = Path(
    "BPC_future/logical_graph/run_reports/"
    "20260614_bpc_future_gat_embedding_sector_wave_smoke_runbook_zh.md"
)
...
def _validation_command(
    *,
    ...
    validation_dir = output_dir / "sector_wave_gat_embedding_capture_validation"
    parts = [
        "PYTHONDONTWRITEBYTECODE=1",
        "PYTHONPATH=.",
        PYTHON,
...
def build_runbook(
    *,
    ...
    validation_report = output_dir / "sector_wave_gat_embedding_capture_validation_zh.md"
    ...
    commands = [
        ...
        {
            "command_type": "gat_embedding_knn_ood_capture_validation",
            "description": "Read capture logs and validate GAT embedding plus kNN/OOD safety shell.",
    ...
    summary = {
        "schema_version": "gat_embedding_sector_wave_smoke_runbook_v1",
        ...
        "candidate_policy": {
            ...
            "embedding_model": str(checkpoint),
        ...
        "goal_complete": False,
        "next_step": (
            "Run the capture command, then run the GAT embedding validation command. "
            "A later online gate can only be considered if no false positives appear "
            "and 5/10 no-regression plus 20-task ROI evidence is collected."
        ),
    }
