#!/usr/bin/env python3
"""Audit where improved/worsened signal appears along the CG trajectory.

The script is read-only.  It separates stage_rows.csv features into temporal
layers: pre-batch, immediate addition, next-RMP movement, and hindsight
trajectory.  This prevents post-hoc signals from being mistaken for
addition-before selector evidence.
"""
...
DEFAULT_OUTPUT_DIR = Path("BPC_future/results/root_cause_trajectory_signal_ladder_20260613")
...
def build_summary(input_path: Path) -> dict[str, Any]:
    ...
    for layer_name, features in FEATURE_LAYERS.items():
        lod = _leave_one_group(rows, "dataset", features)
        loi = _leave_one_group(rows, "instance", features)
        stats = _feature_stats(rows, features)
        layers[layer_name] = {
            "features": list(features),
            "top_features": stats[:6],
            "leave_one_dataset": lod,
            "leave_one_instance": loi,
        }
    pre = layers["pre_batch"]
    immediate = layers["immediate_addition"]
    hindsight = layers["hindsight_trajectory"]
    pre_lod = _compact(pre["leave_one_dataset"])
    immediate_lod = _compact(immediate["leave_one_dataset"])
    hindsight_lod = _compact(hindsight["leave_one_dataset"])
    return {
        ...
        "layer_summary": {
            layer_name: {
                "top_feature": payload["top_features"][0],
                "leave_one_dataset": _compact(payload["leave_one_dataset"]),
                "leave_one_instance": _compact(payload["leave_one_instance"]),
            }
            for layer_name, payload in layers.items()
        },
        "checks": {
            "pre_batch_precision_low": (pre_lod["precision"] or 0.0) < 0.5,
            "immediate_addition_not_enough": (immediate_lod["precision"] or 0.0) < 0.5,
            "hindsight_precision_higher_than_pre": (
                (hindsight_lod["precision"] or 0.0) > (pre_lod["precision"] or 0.0)
            ),
            "hindsight_precision_higher_than_immediate": (
                (hindsight_lod["precision"] or 0.0)
                > (immediate_lod["precision"] or 0.0)
            ),
            "pre_and_immediate_top_are_quantity_signals": (
                pre["top_features"][0]["feature"]
                in {"returned_union_size", "returned_count", "materialized_count", "selected_count"}
                and immediate["top_features"][0]["feature"]
                in {"addition_new_count", "addition_changed_count", "addition_requested_count"}
            ),
        },
    }
