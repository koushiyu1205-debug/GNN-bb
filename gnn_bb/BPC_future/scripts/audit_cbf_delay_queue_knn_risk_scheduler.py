#!/usr/bin/env python3
"""Audit a kNN-risk guarded CBF delay-queue scheduler.

The scheduler first applies the conservative train zero-FP probability
threshold from ``audit_cbf_delay_queue_scheduler.py``.  It then admits a row to
``HIGH_PRIORITY`` only if nearby training rows in online feature space have low
unsafe density.  Otherwise true-RC negative rows remain in ``DELAY_QUEUE``.

This is diagnostic-only and offline.  It never runs BPC/pricing/RMP, never
generates columns, and never creates certificates or official lower bounds.
"""
...
DEFAULT_OUTPUT_DIR = Path(
    "BPC_future/results/cbf_delay_queue_knn_risk_scheduler_audit_20260614"
)
DEFAULT_REPORT = Path(
    "BPC_future/logical_graph/run_reports/"
    "20260614_bpc_future_cbf_delay_queue_knn_risk_scheduler_audit_zh.md"
)
...
def _neighbor_unsafe_fraction(
    ...
    *,
    ...
    if not neighbors:
        return 1.0
    unsafe_count = sum(1 for _distance_value, label in neighbors if int(label) == 0)
...
def _fold_knn_audit(
    *,
    ...
    return {
        ...
        "raw_holdout_metrics_before_knn": raw_holdout_metrics,
    ...
    *,
    ...
    for task_count, indexed in _task_groups(rows).items():
        ...
        folds = _bucket_instance_folds(
            indexed,
            fold_kind="knn_delay_queue_within_scale_leave_one_instance",
        ...
        scale_results.append(
            {
                **base,
                ...
                "reason": "passed_knn_delay_queue_scale_holdout"
                if ready
                else "knn_delay_queue_scale_holdout_not_safe_or_not_productive",
    ...
    for (task_count, family), indexed in _family_groups(rows).items():
        ...
        folds = _bucket_instance_folds(
            indexed,
            fold_kind="knn_delay_queue_within_family_leave_one_instance",
            min_holdout_rows=min_holdout_rows,
            min_train_rows=min_train_rows,
            epochs=epochs,
            lr=lr,
        ...
        family_results.append(
            {
                **base,
                ...
                "must_delay": not ready,
                "reason": "passed_knn_delay_queue_family_holdout"
                if ready
                else "knn_delay_queue_family_holdout_not_safe_or_not_productive",
                "fold_summary": fold_summary,
                "folds": folds,
            }
        )
    ...
    summary = {
        "schema_version": "cbf_delay_queue_knn_risk_scheduler_audit_v1",
