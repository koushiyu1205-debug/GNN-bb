...
class CBFDelayQueueKNNOODSchedulerTests(unittest.TestCase):
    def test_knn_ood_scheduler_keeps_exactness_guards(self) -> None:
    ...
    def test_knn_ood_scheduler_fails_closed_on_certificate_effect_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            dataset = tmp / "cbf_trajectory_gate_transitions.jsonl"
            rows = [
