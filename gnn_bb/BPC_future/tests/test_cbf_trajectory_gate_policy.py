...
from BPC_future.scripts.audit_cbf_trajectory_gate_policy import audit_trajectory_gate_policy
...
def _row(
    *,
    ...
    return {
        ...
        # Deliberately make the one-step label uninformative.  The trajectory
        # audit must use label_horizon_cbf_feasible instead.
        "label_cbf_feasible": 1,
        "label_bad_mode_transition": 0,
        "label_delta_v_nonpositive": 1,
...
class CBFTrajectoryGatePolicyAuditTests(unittest.TestCase):
    def test_trajectory_policy_uses_horizon_labels_and_excludes_leakage(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            dataset = tmp / "cbf_trajectory_gate_transitions.jsonl"
            rows = [
                _row(instance="apollo_greedy_1", task_count=20, horizon_feasible=1, state_dual=0.2, action_count=3),
                _row(instance="apollo_greedy_1", task_count=20, horizon_feasible=0, state_dual=4.0),
                _row(instance="tranq_greedy_1", task_count=20, horizon_feasible=1, state_dual=0.3, action_count=3),
                _row(instance="tranq_greedy_1", task_count=20, horizon_feasible=0, state_dual=5.0),
                _row(instance="very_small", task_count=10, horizon_feasible=1, state_dual=0.4, action_count=2),
                _row(instance="very_small", task_count=10, horizon_feasible=0, state_dual=6.0),
            ]
            dataset.write_text(
            ...
            self.assertEqual(summary["label_counts"], {"0": 3, "1": 3})
            ...
            self.assertNotIn("horizon_delta_v", summary["base_feature_names"])
            self.assertNotIn("label_horizon_cbf_feasible", summary["base_feature_names"])
            ...
            self.assertEqual(summary["gate_role"], "stability_scheduler_not_column_filter")
            ...
            small_scale = [
                item for item in summary["scale_results"] if item["task_count"] < 20
            ]
            self.assertTrue(small_scale)
            self.assertTrue(all(item["must_abstain"] for item in small_scale))

    def test_trajectory_policy_fails_closed_on_certificate_effect_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            dataset = tmp / "cbf_trajectory_gate_transitions.jsonl"
            rows = [
