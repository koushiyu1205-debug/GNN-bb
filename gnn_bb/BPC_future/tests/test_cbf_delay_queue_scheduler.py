...
from BPC_future.scripts.audit_cbf_delay_queue_scheduler import audit_delay_queue_scheduler
...
class CBFDelayQueueSchedulerAuditTests(unittest.TestCase):
    def test_scheduler_keeps_unsafe_negative_in_delay_queue_not_discard(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            ...
            self.assertEqual(
                summary["proof_stage_budget_effect"],
                "none_existing_exact_deadlines_unchanged",
            )
            self.assertTrue(summary["checks"]["delay_queue_does_not_extend_proof_budget"])

            small_scale = [
                item for item in summary["scale_results"] if item["task_count"] < 20
            ]
            self.assertTrue(small_scale)
            self.assertTrue(all(item["must_delay"] for item in small_scale))

    def test_scheduler_fails_closed_on_certificate_effect_rows(self) -> None:
        with tempfile.TemporaryDirectory() as tmpdir:
            tmp = Path(tmpdir)
            dataset = tmp / "cbf_trajectory_gate_transitions.jsonl"
            rows = [
