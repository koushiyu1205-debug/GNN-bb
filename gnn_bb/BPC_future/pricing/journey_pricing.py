...
def _price_journeys_by_profiles(
    ...
    *,
    ...
    if status != "OPTIMAL":
        ...
        if journeys:
            ...
            return JourneyPricingResult(
                journeys=journeys,
                exhausted=False,
                best_reduced_cost=objective,
                generated_sequences=generated,
                evaluated_timed_trips=evaluated,
                candidate_trips=len(profiles),
                selected_trips=max((len(selected) for selected, _obj in selected_candidates), default=0),
                status="INCOMPLETE",
                reason="partial_dp_negative_journey",
                profile_dominance_pruned=profile_dominance_pruned,
                existing_journeys_filtered=existing_filtered,
                profile_cut_penalty_pruned=cut_penalty_pruned,
        ...
        return JourneyPricingResult(
            ...
            selected_trips=0,
            status=status,
            reason=_profile_dp_incomplete_reason(status, dp_stats),
            profile_dominance_pruned=profile_dominance_pruned,
            existing_journeys_filtered=existing_filtered,
            profile_cut_penalty_pruned=cut_penalty_pruned,
            weak_negative_journeys_filtered=weak_filtered,
    ...
    if not journeys:
        ...
        return JourneyPricingResult(
            journeys=[],
            exhausted=exhausted_for_result,
            best_reduced_cost=objective,
            generated_sequences=generated,
            evaluated_timed_trips=evaluated,
            candidate_trips=len(profiles),
    ...
    return JourneyPricingResult(
        ...
        reason="negative_journey" if exhausted else "partial_negative_journey",
        profile_dominance_pruned=profile_dominance_pruned,
        existing_journeys_filtered=existing_filtered,
        profile_cut_penalty_pruned=cut_penalty_pruned,
        weak_negative_journeys_filtered=weak_filtered,
