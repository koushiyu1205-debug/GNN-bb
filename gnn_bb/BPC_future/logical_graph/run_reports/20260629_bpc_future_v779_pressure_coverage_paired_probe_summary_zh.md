# Journey Paired Probe Summary

日期：2026-06-29

## Boundary

This report only summarizes already completed paired replay probes. It does not run BPC / pricing / RMP, and it does not create official bounds or certificates.

## Summary

```text
entry_count = 8
paired_group_count = 2
baseline_entry_count = 2
alternative_entry_count = 6
result_available_entry_count = 2
missing_result_entry_count = 6
observed_alternative_entry_count = 1
valid_observed_alternative_entry_count = 1
target_not_replayed_entry_count = 0
target_pair_not_selected_entry_count = 0
label_counts = {'missing_result': 5, 'neutral_proxy': 1}
production_ready = false
certificate_effect = false
official_bound_effect = false
```

## Groups

- `tranquillitatis_balmer_like_20km_greedy-anchor_randomtw_tasks020_04_seed61311_logical_graph__d1__n1__sel_5,13`
  baseline = None / None
  best_alt = None / None
  gains = wall None, profile None, child_cb_retry None
  target = hit 0, not_replayed 0, pair_not_selected 0
  labels = {'missing_result': 3}
- `tranquillitatis_balmer_like_20km_greedy-anchor_randomtw_tasks020_04_seed61311_logical_graph__d1__n2__sel_5,13`
  baseline = [5, 13] / 005_candidate_selected_d1_n2_5_13_tranquillitatis_balmer_like_20km_greedy-anchor_randomtw_tasks020_04_seed61311_logical_graph
  best_alt = [7, 13] / 006_candidate_alt_d1_n2_r1_7_13_tranquillitatis_balmer_like_20km_greedy-anchor_randomtw_tasks020_04_seed61311_logical_graph
  gains = wall -1.293763, profile 0.0, child_cb_retry 0.0
  target = hit 2, not_replayed 0, pair_not_selected 0
  labels = {'neutral_proxy': 1, 'missing_result': 2}
