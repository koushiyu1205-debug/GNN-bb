...
本报告只审计已生成的 no-certificate-effect smoke 产物 `BPC_future/results/root_cause_active_basis_snapshot_multi20_smoke_20260614`，确认 full active-basis snapshot 采集链路能把 `active_basis_churn_count_before` 和 `rmp_degeneracy_pressure_before` 写入 candidate impact rows。
...
active_basis_payload_count_min = 8
